...
# Функция для загрузки и очистки векторов из JSON файла
def load_and_clean_vectors(file_path):
    ...
    for video_id, data in vectors_data.items():
        ...
        for vector in data['vectors']:
            ...
            if expected_length is None:
                expected_length = len(vector_array)
            elif len(vector_array) != expected_length:
                logging.error(f"Vector length mismatch: {len(vector_array)} (expected {expected_length}) for video ID {video_id}")
                continue  # Пропустить вектор с неправильной длиной
            video_vectors.append(vector_array)
            video_ids.append(video_id)
            valid_vectors.append(vector)  # Добавить вектор в список допустимых

        if valid_vectors:
            cleaned_vectors_data[video_id] = {'vectors': valid_vectors}  # Сохранить только допустимые векторы

    # Сохранить очищенные данные обратно в JSON-файл
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(cleaned_vectors_data, f, indent=4, ensure_ascii=False)
        logging.info(f"Cleaned vectors saved to {file_path}")

    logging.debug(f"Video vectors: {video_vectors}")
    return video_vectors, video_ids

try:
    video_vectors, video_ids = load_and_clean_vectors(vectors_file_path)
    logging.info("Successfully loaded and cleaned vectors.")
except Exception as e:
    logging.error(f"Failed to load and clean vectors: {str(e)}")
    raise
...
if os.path.exists(video_index_file_path):
    ...
    try:
        ...
        logging.error(f"Error adding vectors to Faiss index: {str(e)}")
        raise
