...
logging.basicConfig(
    filename='RANDOM_OLD/subtitles.log',
    level=logging.DEBUG,
...
def is_valid_keyword(keyword):
    ...
    for word in keyword.split():
        if re.fullmatch(f'{vowels}{{2,}}', word) or re.fullmatch(f'{consonants_rus}{{2,}}', word) or re.fullmatch(
            ...
            logging.debug(f"Удалено ключевое слово '{keyword}': содержит непрерывные последовательности гласных или согласных.")
            return False
        ...
        if len(word) < 3 and word not in exceptions:
            logging.debug(f"Удалено ключевое слово '{keyword}': содержит слово длиной менее трех символов, не являющееся исключением.")
            return False
...
# Загрузка JSON-файла
try:
    with open('/mnt/data/subtitles_easyocr.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
...
try:
    with open('/mnt/data/new_subtitles_cleaned.json', 'w', encoding='utf-8') as f:
...
logging.info(f"Обработка завершена. Обновленный файл сохранен как 'new_subtitles_cleaned.json'.")
