...
def user_search_request():
    ...
    try:
        ...
        # Сортировка по количеству совпадений и взвешенной сумме расстояний
        sorted_video_results = sorted(video_results.items(), key=lambda x: (-x[1]['count'], x[1]['total_distance'] / x[1]['count']))

        formatted_video_results = "\n".join(
            [f"{i + 1}. Video ID: {video_id}, Count: {result['count']}, Avg Distance: {result['total_distance'] / result['count']:.2f}\nURL: {result['url']}" for i, (video_id, result) in enumerate(sorted_video_results)])
